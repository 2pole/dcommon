...
namespace DCommon.Mvc.Grid
{
    ...
    {
        private const string DefaultCssClass = "table table-striped dataTable";
        private const string SortingCssClass = "sorting";
        private const string SortingAscCssClass = "sorting_asc";
        private const string SortingDescCssClass = "sorting_desc";
        ...
        protected override void RenderHeaderCellStart(GridColumn<T> column)
        {
            ...
            if (IsSortingEnabled && column.Sortable)
            {
                ...
                string sortClass = isSortedByThisColumn ? (GridModel.SortOptions.Direction == ListSortDirection.Ascending ? SortingAscCssClass : SortingDescCssClass)
                    : SortingCssClass;
                ...
                attributes["data-ajaxPager"] = "true";
                attributes["data-sortMember"] = sortColumnName;
            }
        ...
        protected override void RenderRowStart(GridRowViewData<T> rowData)
        {
            ...
            {
                attributes["class"] = rowData.IsAlternate ? "even" : "odd";
            }
        ...
        {
            string attrs = BuildHtmlAttributes(GridModel.BodyAttributes);

            if (attrs.Length > 0)
                attrs = " " + attrs;

            RenderText(string.Format("<tbody{0}>", attrs));
        }
